...
(function() {
  ...
  function waitForLit() {
    if (checkLitAvailable()) {
      // window.lit is available, define the card
...
function defineCard(LitElement, html, css) {
  class OpenctrolCard extends LitElement {
    ...
    this._latchedKeys = new Set(); // Track all latched keys (not just modifiers)
    ...
    this._fullscreenKeyboardVisible = false;
    this._fullscreenLeftHeld = false;
    this._fullscreenRightHeld = false;
    this._fullscreenMiddleHeld = false;
  ...
  _handleKeyHold(key) {
    // Long press on any key toggles latch (without sending the key)
    // Toggle latch state for the key
    this._toggleKeyLatch(key);
    console.log(`Key ${key} latch toggled. Latched keys:`, Array.from(this._latchedKeys));
  }

  _toggleKeyLatch(key) {
    const isModifier = ["CTRL", "ALT", "SHIFT", "WIN"].includes(key);
    if (isModifier) {
      // Toggle in modifiers set
      if (this._latchedModifiers.has(key)) {
        this._latchedModifiers.delete(key);
        this._latchedKeys.delete(key);
      } else {
        this._latchedModifiers.add(key);
        this._latchedKeys.add(key);
      }
    } else {
      // Toggle in general latched keys set
      if (this._latchedKeys.has(key)) {
        this._latchedKeys.delete(key);
        // Send key up when unlatching
        this._sendKeyUp(key);
      } else {
        this._latchedKeys.add(key);
        // Send key down when latching
        this._sendKeyDown(key);
      }
    }
    this.requestUpdate();
  }
  ...
  async _sendKeyDown(key) {
    // Send key down event for latched keys
    if (!this._entity || !this._isOnline) return;
    try {
      await this.hass.callService("openctrol", "send_key_event", {
        entity_id: this.config.entity,
        key: key,
        down: true,
      });
    } catch (err) {
      console.error(`Failed to send key down for ${key}:`, err);
    }
  }

  async _sendKeyUp(key) {
    // Send key up event for latched keys
    if (!this._entity || !this._isOnline) return;
    try {
      await this.hass.callService("openctrol", "send_key_event", {
        entity_id: this.config.entity,
      ...
      console.error(`Failed to send key up for ${key}:`, err);
    }
  }
  ...
  async _openFullscreenMonitor(monitorId) {
    ...
    document.getElementById("openctrol-fullscreen-keyboard").addEventListener("click", () => {
      // Toggle keyboard panel overlay in fullscreen
      if (this._fullscreenKeyboardVisible) {
        this._fullscreenKeyboardVisible = false;
        const panel = document.getElementById("openctrol-fullscreen-keyboard-panel");
        if (panel) panel.remove();
      } else {
        this._fullscreenKeyboardVisible = true;
        this._showFullscreenKeyboardPanel();
      }
    });
    // Setup fullscreen mouse buttons with lock functionality
    const leftBtn = document.getElementById("openctrol-fullscreen-left");
    const rightBtn = document.getElementById("openctrol-fullscreen-right");
    const middleBtn = document.getElementById("openctrol-fullscreen-middle");
    
    this._setupFullscreenButtonLock(leftBtn, "left", () => {
      this._fullscreenLeftHeld = !this._fullscreenLeftHeld;
      if (this._fullscreenLeftHeld) {
        this._sendPointerButton("left", "down");
        leftBtn.textContent = "Release Left";
        leftBtn.classList.add("active");
      } else {
        this._sendPointerButton("left", "up");
        leftBtn.textContent = "Left Click";
        leftBtn.classList.remove("active");
      }
    });
    
    this._setupFullscreenButtonLock(rightBtn, "right", () => {
      this._fullscreenRightHeld = !this._fullscreenRightHeld;
      if (this._fullscreenRightHeld) {
        this._sendPointerButton("right", "down");
        rightBtn.textContent = "Release Right";
        rightBtn.classList.add("active");
      } else {
        this._sendPointerButton("right", "up");
        rightBtn.textContent = "Right Click";
        rightBtn.classList.remove("active");
      }
    });
    
    this._setupFullscreenButtonLock(middleBtn, "middle", () => {
      this._fullscreenMiddleHeld = !this._fullscreenMiddleHeld;
      if (this._fullscreenMiddleHeld) {
        this._sendPointerButton("middle", "down");
        middleBtn.textContent = "Release Middle";
        middleBtn.classList.add("active");
      } else {
        this._sendPointerButton("middle", "up");
        middleBtn.textContent = "Middle Click";
        ...
        <div class="panel-section">
          ...
          <div class="keyboard-row">
            ${modifiers.map(key => html`
              <button
                class="key-button ${this._latchedKeys.has(key) ? "latched" : ""}"
                @mousedown=${(e) => {
                  e.preventDefault();
                  const isModifier = ["CTRL", "ALT", "SHIFT", "WIN"].includes(key);
                  const timeout = setTimeout(() => {
                    // Long press - toggle latch for any key
                    this._handleKeyHold(key);
                    this._keyHoldTimeouts.delete(key);
                  }, 300); // Reduced to 300ms for consistency
                ...
                @click=${(e) => {
                  e.preventDefault();
                  // Check if key is already latched - if so, unlatch it
                  if (this._latchedKeys.has(key)) {
                    this._toggleKeyLatch(key);
                    return;
                  }
                ...
                @touchstart=${(e) => {
                  ...
                  const timeout = setTimeout(() => {
                    this._handleKeyHold(key);
                    this._keyHoldTimeouts.delete(key);
                  }, 300); // Reduced to 300ms for consistency with mouse buttons
                  this._keyHoldTimeouts.set(key, timeout);
                }}
                @touchend=${(e) => {
                  e.preventDefault();
  ...
  try {
    if (typeof customElements !== 'undefined' && !customElements.get('openctrol-card')) {
      customElements.define("openctrol-card", OpenctrolCard);
      console.log("Openctrol Card custom element registered successfully");
    }
  } catch (error) {
    console.error("Failed to register Openctrol Card custom element:", error);
  }
  ...
  try {
    ...
    console.log("Openctrol Card registered with Home Assistant");
  } catch (error) {
