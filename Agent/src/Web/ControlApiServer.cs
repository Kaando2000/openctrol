...
public sealed class ControlApiServer : IControlApiServer
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Configure Kestrel explicitly - no need to clear URLs as we're configuring Kestrel directly
            ...
            _app.Map("/ws/desktop", async (HttpContext context) =>
        {
            ...
            _logger.Info($"[API] RunTask status: {(_runTask?.Status.ToString() ?? "null")}, IsFaulted: {(_runTask?.IsFaulted ?? false)}");
            
            while (DateTime.UtcNow - startTime < startTimeout)
            {
                if (cancellationToken.IsCancellationRequested)
                {
                    _logger.Error("[API] Cancellation token was cancelled during server startup!");
                    throw new OperationCanceledException("Server startup was cancelled", cancellationToken);
                }
